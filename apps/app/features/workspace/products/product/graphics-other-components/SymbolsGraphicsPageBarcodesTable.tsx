...
import AddBarcodesDialog from "./AddBarcodesDialog";
...
const FILTER_COLUMNS: ListFilterColumn[] = [
  { name: "componentName", label: "Barcode Type", type: "text" },
  { name: "componentDescription", label: "Barcode Data", type: "text" },
...
const columns: ColumnDef<Item>[] = [
  ...
  },
  {
    ...
    enableSorting: true,
    meta: { label: "Barcode Type" },
    header: ({ column }) => (
      <SortableHeader column={column} title="Barcode Type" />
    ),
    cell: ({ row, table }) => {
      const meta = table.options.meta as TableMeta | undefined;
      const diff = meta?.isRedlineView
  ...
  },
  {
    ...
    enableSorting: true,
    meta: { label: "Barcode Data" },
    header: ({ column }) => (
      <SortableHeader column={column} title="Barcode Data" />
    ),
    cell: ({ row, table }) => {
      const meta = table.options.meta as TableMeta | undefined;
      const diff = meta?.isRedlineView
    ...
    <div className="flex flex-1 min-h-0 flex-col overflow-hidden">
      <div className="flex h-10 shrink-0 items-center justify-between gap-2 border-b border-border bg-background p-2 pl-3">
        <div className="flex items-center gap-2">
          ...
          <InfoTooltip content="Add and manage barcodes used on product labels and packaging." />
        </div>
        <div className="flex items-center gap-2">
