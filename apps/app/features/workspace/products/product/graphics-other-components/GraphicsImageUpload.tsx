...
const GRAPHICS_IMAGE_ACCEPT =
  "image/png,image/jpg,image/jpeg,image/gif,image/webp";
const GRAPHICS_IMAGE_HELPER_TEXT = "Supports PNG, JPEG, JPG, GIF, WEBP";

export function GraphicsImageUpload({
  imageUrl,
  label = "Image",
  tooltip = "Upload a reference image for this graphic.",
  setNewImage,
  setRemoveImage,
}: {
  imageUrl?: string;
  ...
  setRemoveImage?: (removed: boolean) => void;
}) {
  ...
  useEffect(() => {
    ...
    if (files.length === 0) {
      setNewImage(null);
      setRemoveImage?.(hadInitialImage);
      return;
    }
    ...
    setNewImage(null);
    setRemoveImage?.(false);
  }, [files, imageUrl, setNewImage, setRemoveImage]);
      ...
      <div className="relative h-20 w-28 shrink-0 overflow-hidden rounded-lg border border-border bg-muted/30">
        {currentImage ? (
          <Image
            ...
            alt={
              files[0]?.preview
                ? "Preview of uploaded graphic image"
                : "Graphic image"
            }
            width={112}
            height={80}
