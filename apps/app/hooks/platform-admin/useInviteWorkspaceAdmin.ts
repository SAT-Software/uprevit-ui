...
export function useInviteWorkspaceAdmin(workspaceId: string) {
  const auth = useAuth();
  ...
  return useMutation({
    mutationFn: async (input: { email: string; name: string }) => {
      ...
      return fetchPlatformAdmin<{ email: string; userId: string }>(
        `/api/platform-admin/workspaces/${workspaceId}/admins/invite`,
        { auth, method: "POST", body: input },
      );
    },
    onSuccess: () => {
      toast.success("Workspace admin invite sent");
      queryClient.invalidateQueries({ queryKey: ["platform-admin"] });
    },
    onError: (error) => {
      toast.error(getErrorMessage(error, "Failed to send workspace admin invite"));
    },
  });
}
